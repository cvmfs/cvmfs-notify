...
CVMFS_NOTIFY_VERSION=$(grep -o "[0-9]\+\.[0-9]\+\.[0-9]\+" apps/cvmfs_notify/src/cvmfs_notify.app.src)
...
$BUILDER_SCRIPT \
    _build/prod/rel/cvmfs_notify/cvmfs_notify-${CVMFS_NOTIFY_VERSION}.tar.gz \
    $(cd ${CVMFS_BUILD_LOCATION}; pwd) \
    ${CVMFS_BUILD_PLATFORM} \
    ${CVMFS_NOTIFY_VERSION} \
    ${PACKAGE_VERSION}
